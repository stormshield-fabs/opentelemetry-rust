...
impl MeterProvider for SdkMeterProvider {
    fn library_meter(&self, scope: InstrumentationScope) -> Meter {
        ...
        if let Ok(mut meters) = self.inner.meters.lock() {
            let meter = meters
                .entry(InstrumentationScope::clone(&scope))
...
mod tests {
    ...
    fn same_meter_reused_same_scope() {
        ...
        let _meter3 = provider.library_meter(scope.clone());
        let _meter4 = provider.library_meter(scope.clone());
        let _meter5 = provider.library_meter(scope);
        ...
        let _meter6 = provider.library_meter(library.clone());

        library.name = "Abc".into();
        let _meter7 = provider.library_meter(library.clone());
